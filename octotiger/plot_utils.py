...
def plot_one_collection(graph_map, labelstrings, axes, yaxisfunction, xaxisfunction, minmax) :
    ...
    for index in range(len(series_keys)):
        ...
        if (xb==0) and (yb==0):
            axes.plot(*list(zip(*values)), markersize=6, marker=next(localmarkers), color=next(localcolours))
        elif (xb!=0) and (yb==0):
            axes.semilogx(*list(zip(*values)), basex=xb, markersize=6, marker=next(localmarkers), color=next(localcolours))
        elif (xb==0) and (yb!=0):
            axes.semilogy(*list(zip(*values)), basey=yb, markersize=6, marker=next(localmarkers), color=next(localcolours))
        elif (xb!=0) and (yb!=0):
            axes.loglog(*list(zip(*values)), basex=xb, basey=yb, markersize=6, marker=next(localmarkers), color=next(localcolours))
        else:
          print("Error, unsupported log/lin options")

    # generate labels for each power of N on the axes, check xb (xbase)
    if (xb!=0):
      ...
      axes.xaxis.set_major_formatter(matplotlib.ticker.FuncFormatter(xaxisfunction))
    ...
    if (yb!=0):
      ...
      axes.yaxis.set_major_formatter(matplotlib.ticker.FuncFormatter(yaxisfunction))
...
#----------------------------------------------------------------------------
def plot_configuration(graph_map, mapnames, axesnames, titlefunction, legendfunction, legendtitlefunction, yaxisfunction, xaxisfunction, minmax, bbox) :
      ...
      for param2_i in range(num_param2):
        ...
        try:
          ...
          plot_one_collection(param2_results,
            ...
            newplot,yaxisfunction,xaxisfunction, minmax)
...
#----------------------------------------------------------------------------
def retrieve_safe(a_map, key1, key2, key3, key4) :
  #print(key1,key2,key3,value[0],value[1])
  found = False

  # create the 3 level deep map entries if they are not present
  if not (key1) in a_map:
    print(key1,key2,key3, "fail 0")
    return None
  if not (key2) in a_map[key1]:
    print(key1,key2,key3, "fail 1")
    return None
  if not (key3) in a_map[key1][key2]:
    print(key1,key2,key3, "fail 2")
    return None
